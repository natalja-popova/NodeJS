...
myApp.post("/books", (req, res) => {
  const book = req.body;

  books.push(book);
  res.status(201).json({ addedBook: book });
});

myApp.put("/books/:id", (req, res) => {
  ...
  res.status(201).json({ message: "Knyga atnaujinta" });
});

myApp.delete("/books/:id", (req, res) => {
  const id = req.params.id;
  ...
  res.status(200).json({ message: `Knyga '${deletedBook.title}' pašalinta` });
});

myApp.delete("/books", (req, res) => {
  books = [];
  res.status(200).json({ message: "Visos knygos pašalintos" });
});
